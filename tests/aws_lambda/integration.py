...
def create_worker_function(client, worker_filename, delay=None):
    ...
    zipf = zipfile.ZipFile('python-worker.zip', 'w', zipfile.ZIP_DEFLATED) #TODO: make this a tempfile
...
def create_orchestrator_function(client, worker_function_name, timeout=None):
    ...
    environment = {
        'Variables': {
            'LAMBDA_WORKER_FUNCTION_NAME': worker_function_name
        }
    }
    ...
    zipf = zipfile.ZipFile('python-orchestrator.zip', 'w', zipfile.ZIP_DEFLATED) #TODO: make this a tempfile
...
def cleanup(client, worker_function_name, orchestrator_function_name):
    ...
    worker_response = client.delete_function(
        FunctionName=worker_function_name
    )
...
def test_integration_success():
    ...
    orchestrator_function_name, response = create_orchestrator_function(client, worker_function_name)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 201
    ...
    worker_response, orchestrator_response = cleanup(client, worker_function_name, orchestrator_function_name)
...
def test_integration_timeout_failure():
    ...
    orchestrator_function_name, response = create_orchestrator_function(client, worker_function_name, timeout=1)
    assert response['ResponseMetadata']['HTTPStatusCode'] == 201
