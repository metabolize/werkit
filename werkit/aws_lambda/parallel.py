...
async def call_worker_service(lambda_worker_function_name, extra_args, _input):
    ...
    response = await loop.run_in_executor(None, partial(client.invoke, FunctionName=lambda_worker_function_name, Payload=json.dumps({'input': _input, 'extra_args': extra_args})))
